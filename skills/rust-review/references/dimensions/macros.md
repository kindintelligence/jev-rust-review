...
Macros are code generators with their own failure modes. Review what they expand to, not just what they look like.
...
- **Parsing edge cases.** Trailing commas, empty input, `$(...),*` versus `$(...),+`, and expression fragments that bind unexpectedly: `$e:expr` followed by tokens.
