...
- 24 cells of each matrix were then run again on commit `e22decc`, and these reports hold the re-run cells. The first group is every C and J cell of `explained_expect` and `symlink_check_then_delete`, whose fixtures were corrected. The second group is the J cells of `check_then_act`, `lock_order_inversion`, `noisy_service_tidy` and `scoped_lock_before_await`. Units in those four fixtures are asked the new `concurrency.lock_order` question. The first-pass cells are kept, outside the repository, under `eval-results/e2e/<tag>/superseded/`.
