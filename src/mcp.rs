...
impl Server {
    ...
    #[tool(
        description = "Verify candidate Rust review findings with TypeSafe Jev before reporting them. For each finding (dimension, file, start_line, end_line, one-sentence claim naming identifiers rather than line numbers, proposed severity) the server re-reads the code itself and returns: `support` (Jev's choice of supported / refuted / insufficient_context, with probabilities), `supported` (the probability of `supported`, which the report bar applies to), Jev's independent `severity` score (level name, p_high_or_above, confidence), a `category` choice (real_defect / remote_risk / debatable_tradeoff / style_preference), and a `verdict`: report | insufficient_context | uncertain | dismiss | not_material | tool_reported. tool_reported means cargo_diagnostics already reported this defect on these lines (`tool` names the lint): drop the finding and let the tool's diagnostic stand. That check needs no API key, so call this tool even when Jev is unavailable. The server sends Jev the claimed lines, their enclosing item, and the definitions of any items the claim names elsewhere in the repository. `unseen` lists names in the claim that Jev was still not shown; while there are any, a claim Jev did not confirm comes back insufficient_context and never dismiss. Every verdict except tool_reported is a second opinion from a model that sees only that, not a ruling. insufficient_context means the claim depends on code outside the excerpt; it is not a refutation. On uncertain or dismiss, re-read the code for what Jev may have seen; drop the finding if you find it, and keep it if you can still demonstrate the concrete failure. not_material means the claim is true only under a condition the code gives no reason to expect (a poisoned lock, a sum of ordinary counts overflowing): leave it out of the report unless you can name the realistic input that reaches it."
    )]
...
impl ServerHandler for Server {
    fn get_info(&self) -> ServerConfig {
        ServerConfig::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions(
                "Rust code review helpers: deterministic tools first, then TypeSafe Jev for what no tool can answer. Call cargo_diagnostics for the compiler and lint facts on the changed lines, then evaluate_rust_changes to find where else to look, inspect the flagged code yourself, then call verify_rust_findings for a second opinion on your candidate findings before reporting them. Never report a defect a tool already reported. Jev numbers are routing signals; report them labelled as Jev's.",
            )
    }
}
