...
const READING_FLAGS: &str = "Each flag is a question to answer by reading the code, in `check`. It is not a finding and most flags come to nothing. Answer the question yourself. Report something only if you can name the input, call sequence or interleaving that fails. Never report a flag because it was flagged. Flags decide what you read first. Review every changed unit yourself, flagged or not. Jev was asked only the narrow questions listed under each unit, about that unit alone. A unit with no flag has not been cleared. A flagged unit can hold a different defect from the one its flag asks about.";
